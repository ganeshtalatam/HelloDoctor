...
import { Building, Factory, Microscope, Users, Award, TrendingUp, Globe } from "lucide-react";
...
const ProfessionalJourney = () => {
  const journeyPhases = [
    {
      ...
      description: "Leading transformative initiatives across international healthcare networks",
    ...
    },
    {
      ...
      description: "Strategic leadership roles in major pharmaceutical and diagnostic companies",
    ...
    },
    {
      ...
      description: "Specialized focus on rare genetic disorders and diagnostic innovation",
  ...
  const industryStats = [
    { label: "Healthcare Sectors", value: "All Major", icon: <Building className="w-6 h-6" /> },
    { label: "Companies Worked", value: "15+", icon: <Factory className="w-6 h-6" /> },
    { label: "Board Positions", value: "25+", icon: <Award className="w-6 h-6" /> },
    { label: "Years Experience", value: "30+", icon: <TrendingUp className="w-6 h-6" /> }
  ];
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="text-center mb-20 animate-fade-up">
          ...
          <p className="text-xl text-muted-foreground max-w-4xl mx-auto leading-relaxed">
            A transformative journey across all major healthcare sectors, from pharmaceutical innovation 
            to global advisory roles, consistently driving excellence and setting industry standards.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-20">
          {industryStats.map((stat, index) => (
            <Card key={index} className="text-center hover-lift animate-scale-up border-0 shadow-lg" style={{ animationDelay: `${index * 0.1}s` }}>
        ...
        <div className="space-y-12 mb-20">
          <h3 className="text-3xl font-bold text-center mb-12">Career Evolution</h3>
          
          {journeyPhases.map((phase, index) => (
            <div key={index} className={`animate-slide-in ${index % 2 === 0 ? '' : 'lg:flex-row-reverse'} flex flex-col lg:flex-row items-center gap-12`} style={{ animationDelay: `${index * 0.2}s` }}>
                ...
                <Card className="bg-gradient-card border-0 shadow-xl hover-lift">
                  <CardHeader className="pb-4">
                    <div className="flex items-center space-x-4">
                      <div className={`w-16 h-16 bg-gradient-to-r ${phase.color} rounded-2xl flex items-center justify-center text-white`}>
                        {phase.icon}
                      </div>
                      <div>
                        <Badge variant="secondary" className="mb-2">{phase.period}</Badge>
                        <h4 className="text-2xl font-bold">{phase.title}</h4>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <p className="text-muted-foreground mb-6 text-lg">{phase.description}</p>
                    <div className="grid md:grid-cols-2 gap-3">
                      {phase.highlights.map((highlight, highlightIndex) => (
                        <div key={highlightIndex} className="flex items-center space-x-3 p-3 bg-white/50 rounded-lg">
              ...
              <div className="hidden lg:flex flex-col items-center">
                <div className={`w-4 h-4 bg-gradient-to-r ${phase.color} rounded-full`}></div>
        ...
        <div>
          <h3 className="text-3xl font-bold text-center mb-12">Current Board Positions & Advisory Roles</h3>
          <div className="grid md:grid-cols-2 gap-8">
            {currentBoardPositions.map((position, index) => (
              <Card key={index} className="hover-lift animate-fade-up border-0 shadow-lg" style={{ animationDelay: `${index * 0.1}s` }}>
                <CardContent className="p-8">
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-1">
                      <h4 className="font-bold text-lg mb-2">{position.organization}</h4>
                      <Badge className="mb-3">{position.role}</Badge>
                      <p className="text-muted-foreground text-sm">{position.focus}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          <Card className="bg-gradient-hero text-white border-0 shadow-2xl max-w-4xl mx-auto">
            <CardContent className="p-12">
              ...
              <p className="text-xl leading-relaxed opacity-90">
                ...
                Active leadership in Industry Advisory Boards and Ethics Committees, contributing to 
                strategic decision-making and ethical healthcare practices worldwide."
              </p>
            </CardContent>
          </Card>
        </div>
